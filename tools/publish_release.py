...
Chạy: python tools/publish_release.py --version 1.0.4 --notes "Tính năng A" "Tính năng B" --push
"""
...
def main() -> None:
    ...
    parser.add_argument("--version", required=True, help="So phien ban moi (VD: 1.0.4)")
    parser.add_argument("--notes", nargs="+", help="Danh sach ghi chu cap nhat / Changelog")
    parser.add_argument(
        "--url-base",
        default="https://github.com/dudinhnhat1993/pdf-batch-renamer/releases/download",
        help="Base URL tai file",
    )
    parser.add_argument("--push", action="store_true", help="Tu dong git commit, tag va push len GitHub")
    ...
    # Sync releases/version.json
    (ROOT / "releases").mkdir(exist_ok=True)
    (ROOT / "releases" / "version.json").write_text(json.dumps(manifest, indent=2, ensure_ascii=False), encoding="utf-8")
    ...
    if args.push:
        print(f"\n=== Dang tu dong day code va Tag v{new_ver} len GitHub ===")
        try:
            subprocess.run(["git", "add", "."], cwd=str(ROOT), check=True)
            subprocess.run(["git", "commit", "-m", f"chore(release): v{new_ver}"], cwd=str(ROOT), check=False)
            subprocess.run(["git", "tag", "-a", f"v{new_ver}", "-m", f"Release v{new_ver}"], cwd=str(ROOT), check=False)
            subprocess.run(["git", "push", "origin", "main", "--tags"], cwd=str(ROOT), check=True)
            print(f"[THANH CONG] Da push code va Tag v{new_ver} len GitHub!")
        except Exception as e:
            print(f"[WARN] Git push error: {e}")
